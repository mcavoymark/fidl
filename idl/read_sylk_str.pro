;Copyright 4/6/01 Washington University.  All Rights Reserved.
;read_sylk_str.pro  $Revision: 1.6 $
...
FUNCTION GetSylkCellContents, szFileLine, fUseLongs, fUseDoubles
    ...
    IF (posCellContents NE -1) THEN BEGIN
        ...
        ;START171020
        if szCellContents eq '#NULL!' then return,szCellContents 
        ...
        IF (STRPOS(szCellContents, 34B) EQ 0) THEN BEGIN
            ...
            mdefCellContents = STRMID(szCellContents, 1,STRPOS(szCellContents, ";", 1) - 2)
            ;print,'szFileLine=',szFileLine
            ;print,'szCellContents=',szCellContents
            ;print,'mdefCellContents=',mdefCellContents
            ;print,'strlen(mdefCellContents)=',strlen(mdefCellContents)
...
;FUNCTION READ_SYLK_STR, Infile, STARTROW = iStartRow, STARTCOL = iStartCol, $
;    NROWS = nRows, NCOLS = nCols, ARRAY = fArray, COLMAJOR = fColMajor, $
;    USEDOUBLES = fUseDoubles, USELONGS = fUseLongs 
FUNCTION READ_SYLK_STR, Infile, STARTROW = iStartRow, STARTCOL = iStartCol, $
    NROWS = nRows, NCOLS = nCols, ARRAY = fArray, COLMAJOR = fColMajor, $
    USEDOUBLES = fUseDoubles, USELONGS = fUseLongs ,SKIP_FIRST_CELL=skip_first_cell
;
;+
; NAME:
;   READ_SYLK
...
;
    ...
    ; If there was no valid pointer in the array of pointers, make the return data type bytes
    ...
    IF (fArray) THEN BEGIN
        if iMax gt 1 and jMax gt 1 then begin
            ReturnData = strarr(iMax,jMax)
            FOR i = 0, iMax - 1 DO BEGIN
                FOR j = 0, jMax - 1 DO BEGIN
                    IF arrPointers[i, j] NE nullPtr THEN BEGIN

                        ;value = string(*(arrPointers[i, j]))
                        ;START150506
                        value = trim(*(arrPointers[i, j]))

                        ;print,'*(arrPointers[i, j])=',*(arrPointers[i, j])
                    ENDIF ELSE BEGIN
                        value = '' 
                    ENDELSE
                    ;print,'value=',value
            ...
            FOR i = 0, iMax - 1 DO BEGIN
                FOR j = 0, jMax - 1 DO BEGIN
                    ;IF (arrPointers[i, j] NE nullPtr) THEN count = count + 1 
                    if keyword_set(SKIP_FIRST_CELL) and i eq 0 and j eq 0 then begin
                        ;do nothing

                    ;endif else IF (arrPointers[i, j] NE nullPtr) THEN count = count + 1 
                    ;START170616
                    endif else count = count + 1 

                ENDFOR
            ENDFOR
            ReturnData = strarr(count)
            count = 0
            FOR i = 0, iMax - 1 DO BEGIN
                FOR j = 0, jMax - 1 DO BEGIN
                    ;IF (arrPointers[i, j] NE nullPtr) THEN BEGIN
                    ;    ReturnData[count] = string(*(arrPointers[i, j]))
                    ;    count = count + 1
                    ;ENDIF
                    if keyword_set(SKIP_FIRST_CELL) and i eq 0 and j eq 0 then begin
                        ;do nothing
                    endif else IF (arrPointers[i, j] NE nullPtr) THEN BEGIN

                        ;ReturnData[count] = string(*(arrPointers[i, j]))
                        ;START150506
                        ReturnData[count] = trim(*(arrPointers[i, j]))
