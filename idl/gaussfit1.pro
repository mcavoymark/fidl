...
;	F = A(0)*EZ + A(3) + A(4)*X + A(5)*X^2 ;FUNCTIONS. ;***JMO 3/18/94
	F = A(0)*EZ 
...
;
; 		F(x) = A0*EXP(-z^2/2) + A3 + A4*x + A5*x^2 ; Original version
; 		F(x) = A0*EXP(-z^2/2) ; Modified version
; 			and
;		z=(x-A1)/A2
;
...
;a=fltarr(6)			;coefficient vector *** JMO 3/17/94
