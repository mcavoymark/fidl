...
int i,j,k,nsim=1000,nthresh=0,SunOS_Linux,*count,*index=NULL,nreg,lizard;
size_t i1,j1,k1;
unsigned long s=0;
float fwhm=0.;
...
if(!(count=malloc(sizeof*count*(ms->lenbrain+1)*nthresh))) {
    printf("fidlError: Unable to malloc count\n");
    exit(-1);
    }
for(i=0;i<(ms->lenbrain+1)*nthresh;i++) count[i]=0;
