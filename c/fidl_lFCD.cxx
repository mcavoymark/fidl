...
for(size_t i1=0;i1<files->nfiles;++i1){
    ...
    //for(i=0;i<ms.lenbrain;++i)if(std::isnan(sd[ms.brnidx[i]]))std::cout<<"sd["<<ms.brnidx[i]<<"]="<<sd[ms.brnidx[i]]<<std::endl;
    //for(i=0;i<ms.lenbrain;++i)if(std::abs(mean[ms.brnidx[i]])<=(double)UNSAMPLED_VOXEL)std::cout<<"sd["<<ms.brnidx[i]<<"]="<<sd[ms.brnidx[i]]<<std::endl;
    //START210423
    //std::vector<int>::iterator it; 
    for(i=0;i<ms.lenbrain;++i)if(std::isnan(sd[ms.brnidx[i]])||std::abs(sd[ms.brnidx[i]])<=(double)UNSAMPLED_VOXEL){
        //std::cout<<"ms.brnidx["<<i<<"]="<<ms.brnidx[i]<<std::endl;
        //it=se.brnidx.begin()+i;
        //it=se.brnidx.begin()+i-ms.lenbrain+se.brnidx.size();
        //std::cout<<"*it="<<*it<<std::endl;
        //se.brnidx.erase(se.brnidx.begin()+i);
        se.brnidx.erase(se.brnidx.begin()+i-ms.lenbrain+se.brnidx.size());
        //se.nbrnidx--;
        }
    //std::cout<<"ms.lenbrain="<<ms.lenbrain<<" se.nbrnidx="<<se.nbrnidx<<" se.brnidx.size()="<<se.brnidx.size()<<std::endl;
    ...
    if(!meanonly){
        ...
        //for(i=0;i<ms.lenbrain;++i)tf[ms.brnidx[i]]=(float)lFCD[i];
        //for(i=0;i<se.nbrnidx;++i)tf[se.brnidx[i]]=(float)se.lFCD[i];
        ...
        //for(i=0;i<ms.lenbrain;++i)tf[ms.brnidx[i]]=(float)log(lFCD[i]);
        //for(i=0;i<se.nbrnidx;++i)tf[se.brnidx[i]]=(float)log(se.lFCD[i]);
        for(size_t i=0;i<se.brnidx.size();++i)tf[se.brnidx[i]]=(float)log(se.lFCD[i]);
        str2=str+"_loglFCD.nii";
        if(!(nifti_write_setdim4to1((char*)str2.c_str(),files->files[i1],tf)))exit(-1);
        str3="gzip -f "+str2;
        if(system(str3.c_str())==-1)std::cout<<"fidlError: "<<str3<<std::endl;else std::cout<<"Output written to "<<str2+".gz"<<std::endl;

        //for(i=0;i<ms.lenbrain;++i)tf[ms.brnidx[i]]=(float)lFCDpearson[i];
        //for(i=0;i<se.nbrnidx;++i)tf[se.brnidx[i]]=(float)se.lFCDpearson[i];
...
//for(i=0;i<ms.lenbrain;++i)tf[ms.brnidx[i]]=tdmn[i]?(float)(meanloglFCD[i]/(double)tdmn[i]):0.;
...
//for(i=0;i<ms.lenbrain;++i)tf[ms.brnidx[i]]=tdmn[i]?(float)tanh(meanlFCDpearson[i]/(double)tdmn[i]):0.;
...
if(MARMSm){
    //for(i=0;i<ms.lenbrain;++i)tf[ms.brnidx[i]]=tdmn[i]?(float)(wmeanloglFCD[i]/(double)wtmd[i]):0.;
    ...
    //for(i=0;i<ms.lenbrain;++i)tf[ms.brnidx[i]]=tdmn[i]?(float)tanh(wmeanlFCDpearson[i]/(double)wtmd[i]):0.;
