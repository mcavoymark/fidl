...
static char rcsid[] = "$Header: /home/hannah/mcavoy/idl/clib/RCS/compute_glm.c,v 12.116 2009/12/28 22:43:11 mcavoy Exp $";

int linmod(Files_Struct *file_names,char *in_glm_file,char *out_glm_file_in,Files_Struct *xform_file,char *mask_file,int mode,
    ...
    int *roi,int atlas,int SunOS_Linux,int bigendian,int lcATyfile);
...
int	i,j,num_files=0,mode,nF=0,stat,nseed=0,print_ATy=(int)FALSE,nt4_identify=0,*t4_identify,nxform_file=0,num_region_files=0,
        num_regions=0,*roi=NULL,atlas=0,bigendian=1,SunOS_Linux,lcATyfile=0;
        /* lcdontswap=0,swapbytes */
...
if(!seed) {
    fflush(stdout);
    if(!num_files) {
        printf("Error: You have not provided any data. Need to use -bold_files  Abort!\n");
        exit(-1);
        }
    printf("BOLD files:\n");
...
if(lcdontswap) swapbytes=0;
if(!swapbytes) printf("Output will %sbe swapped.\n",swapbytes?"":"not ");
#endif
...
if(!linmod(bold_files,in_glm_file,out_glm_file,xform_file,mask_file,mode,fwhm,nF,cF_tests,seed,print_ATy,t4_identify,num_regions,
    region_files,roi,atlas,SunOS_Linux,bigendian,lcATyfile)) {
