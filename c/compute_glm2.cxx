...
char *in_glm_file=NULL,*out_glm_file=NULL,*mask_file=NULL,*seed_filename,*scratchdir=NULL,*uncompress=NULL,string[MAXNAME],
    atlas[7]; 
float	fwhm=0.;
int i,j,num_bold_files=0,mode,nseed=0,print_ATy=(int)FALSE,nt4_identify=0,*t4_identify=NULL,nxform_file=0,num_region_files=0,
...
for(i=1;i<argc;i++) {
    ...
    if(!strcmp(argv[i],"-bold_files") && argc > i+1) {
        ...
        if(num_bold_files) {
            ...
            if(!strcmp(strptr,".conc")) {
                if(num_bold_files>1) {printf("fidlError: Only set up to handle a single conc. Abort!\n");fflush(stdout);exit(-1);}
                if(!(bold_files=read_conc(1,&argv[++i]))) exit(-1);
                }

            /*else if(!strcmp(strptr,".img")){*/
            /*START150223*/
            else if(!strcmp(strptr,".img")||!strcmp(strptr,".nii")){
    ...
    if(!strcmp(argv[i],"-bold_files") && argc > i+1) {
        ...
        if(!strcmp(strptr,".conc")){
            ...
            printf("fidlError: -bold_files not *.conc, *.img, *.nii or *.nii.gz. Abort!\n");fflush(stdout);
            exit(-1);
            }
        i+=num_bold_files;
        }
    ...
    else if(!strcmp(argv[i],"-mask") && argc > i+1 && strchr(argv[i+1],'-') != argv[i+1])
        mask_file = argv[++i];
...
#if 0
if(!linmod5(bold_files,in_glm_file,out_glm_file,xform_file,mask_file,mode,fwhm,seed,t4_identify,num_regions,region_files,
    roi,atlas,SunOS_Linux,bigendian,lccompress,lcdetrend,scratchdir,lccompressb,lcvoxels,lcR2)) {
    printf("fidlError: Problem in linmod5. Abort!\n");exit(-1);
    }
#endif
//START160906
