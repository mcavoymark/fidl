/* Copyright 3/22/19 Washington University.  All Rights Reserved.
   linmod6.cxx  $Revision: 1.6 $ */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <math.h>
                     ...
                     compute an F for the main effect. */
{
    ...
    if(!region_files) {
        ...
        //START190910
        fprintf(lcdetrend?stderr:stdout,"linmod6 lcflipreg = %d\n",lcflipreg);
        ...
        for(size_t j=0;j<region_files->nfiles;++j){
            ...
            //START190319
            //if(!(reg[j]=get_reg(region_files->files[j],0,(float*)NULL,0,SunOS_Linux,(char**)NULL)))exit(-1);
            //START190327
            //if(!(reg[j]=get_reg(region_files->files[j],0,(float*)NULL,0,SunOS_Linux,(char**)NULL,lcflipreg)))exit(-1);
            //START190910
            //if(!(reg[j]=get_reg(region_files->files[j],0,(float*)NULL,0,(char**)NULL,lcflipreg,0)))exit(-1);
            //START200825
    ...
    fprintf(lcdetrend?stderr:stdout,"linmod6 lcflip=%d cf_flip=%d *********\n",lcflip,cf_flip); 
    if(mask_file) 
        fprintf(lcdetrend?stderr:stdout,"linmod6 MASK %d%d%d  BOLDS %d%d%d *********\n",c_orient[0],c_orient[1],c_orient[2],dp->c_orientall[0],dp->c_orientall[1],
            dp->c_orientall[2]); 
    if(region_files) 
        fprintf(lcdetrend?stderr:stdout,"linmod6 REGIONS %d%d%d  BOLDS %d%d%d *********\n",rbf->c_orient[0],rbf->c_orient[1],rbf->c_orient[2],dp->c_orientall[0],
            dp->c_orientall[1],dp->c_orientall[2]); 
    ...
    //START191008
    for(j=i=0;i<vol;i++)if(maskf[i])j++;
    fprintf(lcdetrend?stderr:stdout,"%d voxels will be processed\n",j);fflush(lcdetrend?stderr:stdout);
