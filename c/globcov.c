/* Copyright 11/08/05 Washington University.  All Rights Reserved.
   globcov.c  $Revision: 1.43 $ */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <math.h>
...
int globcov_guts(Files_Struct *fi,Dim_Param2 *dp,int *valid_frms,double *globcov,int *scalingv,
    ...
    if(!(mean=malloc(sizeof*mean*rbf->nreg*weightsc))) {
        printf("fidlError: Unable to malloc mean in globov_guts\n");
        return 0;
        }
    if(!(max=malloc(sizeof*max*rbf->nreg*weightsc))) {
        printf("fidlError: Unable to malloc max in globov_guts\n");
        return 0;
        }
    if(!(min=malloc(sizeof*min*rbf->nreg))) {
    ...
    if(scalingv[0]==5) {
        ncol = 2;
        if(!(slope=malloc(sizeof*slope*dp->tdim_total))) {
            printf("fidlError: Unable to malloc slope in globcov_guts\n");
            return 0;
            }
        if(!(Astack=malloc(sizeof*Astack*dp->tdim_max*ncol))) {
            printf("fidlError: Unable to malloc Astack in globcov_guts\n");
            return 0;
            }
        if(!(QRstack=malloc(sizeof*QRstack*dp->tdim_max*ncol))) {
            printf("fidlError: Unable to malloc QRstack in globcov_guts\n");
            return 0;
            }
        if(!(bstack=malloc(sizeof*bstack*dp->tdim_max))) {
            printf("fidlError: Unable to malloc bstack in globcov_guts\n");
            return 0;
            }
        if(!(residualstack=malloc(sizeof*residualstack*dp->tdim_max))) {
        ...
        if(!(den=malloc(sizeof*den*rbf->nreg))) {
            printf("fidlError: Unable to malloc den in globov_guts\n");
            return 0;
            }
        if(!(r=malloc(sizeof*r*rbf->nreg*rbf->nreg))) {
    ...
    if(xform_file) {
        if(!(t4=malloc(sizeof*t4*(size_t)T4SIZE*xform_file->nfiles))) {
            printf("fidlError: Unable to malloc t4\n");
            return 0;
            }
        if(!(A_or_B_or_U=malloc(sizeof*A_or_B_or_U*xform_file->nfiles))) {
    ...
    if(nbold_index) if(!(temp_float=malloc(sizeof*temp_float*nbold_index))) {
