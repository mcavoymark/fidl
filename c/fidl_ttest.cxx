/* Copyright 2/19/18 Washington University.  All Rights Reserved.
   fidl_ttest.cxx  $Revision: 1.91 $ */
...
double *zstat,*dferror_stack,*threshold=NULL,*tstat,*tstat_uneqvar=NULL,*df_uneqvar=NULL,mag,mag2,difference,*avg1=NULL,avg2,
    ...
    *RvoxRmap=NULL,*coor=NULL,sumLvoxLmap,sumLvoxRmap,sumRvoxLmap,sumRvoxRmap,*col,*row,*slice,c0,c1LR,c1RL,cov00,cov01,cov11,
    sumsq,a1L,a2L,a1R,a2R,maga1,maga12,maga2,maga22,avga1,vara1,avga2,vara2,*sem=NULL,*sem_uneqvar=NULL,*numerator=NULL,*tstat_p,
    *dferror_stack_p,
    *sem_p;
...
for(i=1;i<argc;i++) {
    if(!strcmp(argv[i],"-driver") && argc > i+1)
        ...
        if(!(harolds_num=(int*)malloc(sizeof*harolds_num*nharolds_num))) {
	...
	if(k != 3) {
	    ...
	    exit(-1);
            ...
            if(!(fidl_voxel=(int*)malloc(sizeof*fidl_voxel*3))) {
	...
	lc_print_cov_matrix = (int)TRUE;
        ...
        if(!(null=(double*)malloc(sizeof*null*nnull))) {
...
if(nnull) {
    ...
    if(!(null=(double*)malloc(sizeof*null*td->ntests))) {
        printf("Error: Unable to malloc null\n");
        exit(-1);
        }
...
if(!(files_ptr=(char**)malloc(sizeof*files_ptr*td->total_nfiles))) {
    printf("fidlError: Unable to malloc files_ptr\n");
    exit(-1);
    }
if(!(n4dfps1=(int*)malloc(sizeof*n4dfps1*td->ntests))) {
    printf("fidlError: Unable to malloc n4dfps1\n");
    exit(-1);
    }
for(i=0;i<td->ntests;i++) n4dfps1[i]=0;
if(!(n4dfps2=(int*)malloc(sizeof*n4dfps2*td->ntests))) {
    printf("fidlError: Unable to malloc n4dfps2\n");
    exit(-1);
    }
...
if(!lccleanup_only) {
    if(!(df=(int*)malloc(sizeof*df*td->ntests))) {
        printf("fidlError: Unable to malloc df\n");
        exit(-1);
        }
    if(!(subtract=(int*)malloc(sizeof*subtract*td->ntests))) {
        printf("fidlError: Unable to malloc subtract\n");
        exit(-1);
        }
    if(!(lc_test_type=(int*)malloc(sizeof*lc_test_type*td->ntests))) {
        printf("fidlError: Unable to malloc lc_test_type\n");
        exit(-1);
        }
    ...
    if(!n4dfps) {
        lenbrain=lenvol=1;
        if(!(data1=(double*)malloc(sizeof*data1*max1))) {
            printf("fidlError: Unable to malloc data1\n");
            exit(-1);
            }
        if(lc0||lc1||lc2||lc5||lc7) {
            if(!(data2=(double*)malloc(sizeof*data2*max2))) {
                printf("fidlError: Unable to malloc data2\n");
                exit(-1);
                }
            }
        }
        ...
        lenvol = dp->volall;
        if(!(ms.get_mask(mask_file,lenvol,(int*)NULL,SunOS_Linux,(LinearModel*)NULL,0))) exit(-1);
        brnidx=ms.get_brnidx(lenbrain,how_many);
        get_atlas(how_many,atlas);
        if(atlas[0]) {
            if(!(ap=get_atlas_param(atlas,filetype==(int)NIFTI?glm->ifh:NULL,(char*)NULL)))exit(-1);
            ifh->dim1 = ap->xdim;
            ifh->dim2 = ap->ydim;
            ifh->dim3 = ap->zdim;
        ...
        if(!(temp_float=(float*)malloc(sizeof*temp_float*j))) {
            printf("fidlError: Unable to malloc temp_float\n");
            exit(-1);
            }
        if(!(temp_float2=(float*)malloc(sizeof*temp_float2*j))) {
        ...
        if(lc6) {
            if(lenvol!=lenbrain) {
                ...
                if(!(brnidx=(int*)malloc(sizeof*brnidx*lenvol))) {
            ...
            if(!(mm1=(Memory_Map**)malloc(sizeof*mm1*max1))) {
                printf("Error: Unable to malloc mm1\n");
                exit(-1);
                }
            if(lc0||lc1||lc2||lc5||lc7) {
                if(!(mm2=(Memory_Map**)malloc(sizeof*mm2*max2))) {
        ...
        if(lc_Z_monte_carlo||lc_T_monte_carlo) {
            if(!(mcmc=monte_carlo_mult_comp_init(ifh->dim1,ifh->dim2,ifh->dim3,lenbrain,brnidx))) exit(-1);
            } 
        }
    ...
    if(lc3||lc4||lc7) {
        if(!(behav_var=(double*)malloc(sizeof*behav_var*max2))) {
            printf("fidlError: Unable to malloc behav_var\n");
            exit(-1);
            }
        }
    ...
    if(lc0||lc1||lc2){
        ...
        if(!(sem=(double*)malloc(sizeof*sem*lenvol))) {
            printf("fidlError: Unable to malloc sem\n");
            exit(-1);
            }
        }
    if(lc0||lc1||lc2||lc5||lc6) {
        if(!(avg1=(double*)malloc(sizeof*avg1*lenvol))) {
            printf("fidlError: Unable to malloc avg1\n");
            exit(-1);
            }
        if(!(var1=(double*)malloc(sizeof*var1*lenvol))) {
            printf("fidlError: Unable to malloc var1\n");
            exit(-1);
            }
        }
    if(lc2||lc7) {
        if(!(tstat_uneqvar=(double*)malloc(sizeof*tstat_uneqvar*how_many))) {
            printf("fidlError: Unable to malloc tstat_uneqvar\n");
            exit(-1);
            }
        if(!(df_uneqvar=(double*)malloc(sizeof*df_uneqvar*how_many))) {
            printf("fidlError: Unable to malloc df_uneqvar\n");
            exit(-1);
            }

        //START190220
        if(!(sem_uneqvar=(double*)malloc(sizeof*sem_uneqvar*lenvol))) {
            printf("fidlError: Unable to malloc sem_uneqvar\n");
            exit(-1);
            }
        if(lc2)if(!(numerator=(double*)malloc(sizeof*numerator*lenvol))) {
            printf("fidlError: Unable to malloc numerator\n");
            exit(-1);
            }

        }
    else if(lc3||lc4) {
        if(!(r=(double*)malloc(sizeof*r*lenvol))) {
            printf("fidlError: Unable to malloc r\n");
            exit(-1);
            }
        }
    if(lc4) {
        if(!(Xblock=(double*)malloc(sizeof*Xblock*nfiles_max*subtract_max))) {
            printf("fidlError: Unable to malloc Xblock\n");
            exit(-1);
            }
        if(!(Xblockcopy=(double*)malloc(sizeof*Xblockcopy*nfiles_max*subtract_max))) {
            printf("fidlError: Unable to malloc Xblockcopy\n");
            exit(-1);
            }
        if(!(Yblock=(double*)malloc(sizeof*Yblock*nfiles_max))) {
            printf("fidlError: Unable to malloc Yblock\n");
            exit(-1);
            }
        if(!(taublock=(double*)malloc(sizeof*taublock*subtract_max))) {
            printf("fidlError: Unable to malloc taublock\n");
            exit(-1);
            }
        if(!(Bblock=(double*)malloc(sizeof*Bblock*subtract_max))) {
            printf("fidlError: Unable to malloc Bblock\n");
            exit(-1);
            }
        if(!(residualblock=(double*)malloc(sizeof*residualblock*nfiles_max))) {
            printf("fidlError: Unable to malloc residualblock\n");
            exit(-1);
            }
        if(!(XTXm1block=(double*)malloc(sizeof*XTXm1block*subtract_max))) {
            printf("fidlError: Unable to malloc XTXm1block\n");
            exit(-1);
            }
        if(!(XTXblock=(double*)malloc(sizeof*XTXblock*subtract_max*subtract_max))) {
            printf("fidlError: Unable to malloc XTXblock\n");
            exit(-1);
            }
        }
    if(lc6) {
        if(!(LvoxLmap=(double*)malloc(sizeof*LvoxLmap*lenbrain))) {
            printf("fidlError: Unable to malloc LvoxLmap\n");
            exit(-1);
            }
        if(!(LvoxRmap=(double*)malloc(sizeof*LvoxRmap*lenbrain))) {
            printf("fidlError: Unable to malloc LvoxRmap\n");
            exit(-1);
            }
        if(!(RvoxLmap=(double*)malloc(sizeof*RvoxLmap*lenbrain))) {
            printf("fidlError: Unable to malloc RvoxLmap\n");
            exit(-1);
            }
        if(!(RvoxRmap=(double*)malloc(sizeof*RvoxRmap*lenbrain))) {
            printf("fidlError: Unable to malloc RvoxRmap\n");
            exit(-1);
            }
    ...
    if(lc7) {
        if(!(tf3=(float*)malloc(sizeof*tf3*lenvol))) {
            printf("fidlError: Unable to malloc tf3\n");
            exit(-1);
            }
        if(!(tf4=(float*)malloc(sizeof*tf4*lenvol))) {
            printf("fidlError: Unable to malloc tf4\n");
            exit(-1);
            }
        }
    ...
    if(!(dferror_stack=(double*)malloc(sizeof*dferror_stack*lenvol))) {
        printf("fidlError: Unable to malloc dferror_stack\n");
        exit(-1);
        }
    if(!(nsubjects1=(int*)malloc(sizeof*nsubjects1*lenvol))) {
        printf("fidlError: Unable to malloc nsubjects1\n");
        exit(-1);
        }
    if(!(nsubjects2=(int*)malloc(sizeof*nsubjects2*lenvol))) {
        printf("fidlError: Unable to malloc nsubjects2\n");
        exit(-1);
        }
    if(!(lt2subjects_voxels_idx=(int*)malloc(sizeof*lt2subjects_voxels_idx*lenvol))) {
        printf("fidlError: Unable to malloc lt2subjects_voxels_idx\n");
        exit(-1);
        }
    if(!(tstat=(double*)malloc(sizeof*tstat*lenvol))) {
        printf("fidlError: Unable to malloc tstat\n");
        exit(-1);
        }
    if(!(zstat=(double*)malloc(sizeof*zstat*lenvol))) {
        printf("fidlError: Unable to malloc zstat\n");
        exit(-1);
        }
    if(!(work=(int*)malloc(sizeof*work*lenvol))) {
        ...
        if(text || lc_statview || !n4dfps) {
            ...
            if(lc_test_type[t]==6) {
                ...
                if(n4dfps1[t]!=n4dfps2[t]) {
                    ...
                    if(diff){
                        if(!(idx=(int*)malloc(sizeof*idx*lenvol))){
            ...
            if(Nimage_name) {
                for(i=0;i<(lc_test_type[t]==2?2:1);i++) {
                    ...
                    if(lc_test_type[t]==5)
                        for(j=0;j<lenbrain;j++) temp_float[brnidx[j]] = (float)nsubjects2[j];
                    else if(lc_test_type[t]==2) {
                        if(!i){for(j=0;j<lenbrain;j++)temp_float[brnidx[j]]=(float)nsubjects1[j];strcpy(string,"1");}
                        else  {for(j=0;j<lenbrain;j++)temp_float[brnidx[j]]=(float)nsubjects2[j];strcpy(string,"2");}
                        }
                    else
                        for(j=0;j<lenbrain;j++) temp_float[brnidx[j]] = (float)(nsubjects1[j]+nsubjects2[j]);
...
//STARTHERE
                            ...
                            //START190220
                            tstat[i] = sp2 < var_thresh ? (double)UNSAMPLED_VOXEL :
                                (numerator[i]=avg1[i]-avg2)/ (sem[i]=sqrt(sp2*(double)(1./nsubjects1[i]+1./nsubjects2[i])));
                            ...
                            if(nsubjects1[i]>1) {
                                ...
                                //START190220
                                tstat_uneqvar[i]=(var1[i]+var2)<var_thresh?(double)UNSAMPLED_VOXEL:
                                    numerator[i]/(sem_uneqvar[i]=sqrt(sum));
            ...
            for(i=0;i<loop;i++) {
                ...
                #endif
                //START190220
                tstat_p=!i?tstat:tstat_uneqvar;
                dferror_stack_p=!i?dferror_stack:df_uneqvar;
                sem_p=!i?sem:sem_uneqvar;
                t_to_z(tstat_p,zstat,lenvol,dferror_stack_p,work);
                ...
                if(!fprn) {
                    ...
                    if(lc_test_type[t]==3||lc_test_type[t]==4) {
                        for(j=0;j<lenbrain;j++) temp_float[brnidx[j]] = (float)r[j];
                    ...
                    if(lc_T_uncorrected || lc_T_monte_carlo) {
                        ...
                        //START190220
                        for(j=0;j<num_voxels_lt2subjects;j++) tstat_p[lt2subjects_voxels_idx[j]] = (double)UNSAMPLED_VOXEL;
                    ...
                    if(lc_T_uncorrected) {
                        ...
                        #if 0
                        for(j=0;j<lenbrain;j++) temp_float[brnidx[j]] = (float)tstat[j];
                        w1->temp_double=tstat;
                        #endif
                        //START190220
                        for(j=0;j<lenbrain;j++) temp_float[brnidx[j]] = (float)tstat_p[j];
                        w1->temp_double=tstat_p;
                    ...
                    if(lc_Z_uncorrected) {
                        for(j=0;j<lenbrain;j++) temp_float[brnidx[j]] = (float)zstat[j];
                        w1->temp_double=zstat;
                        if(!write1(string_z,w1))exit(-1);
                        printf("Z statistics written to %s\n",string_z);
                        if(lc_test_type[t]==0||lc_test_type[t]==1) {
                            for(j=0;j<lenbrain;j++) temp_float[brnidx[j]] = (float)avg1[j];
                            w1->temp_double=avg1;
                            if(!write1(avgstr,w1))exit(-1);
                            printf("Numerator of T test written to %s\n",avgstr);
                            for(j=0;j<lenbrain;j++) temp_float[brnidx[j]] = (float)var1[j];
                            w1->temp_double=var1;
                    ...
                    if((lc_Z_monte_carlo||lc_T_monte_carlo)){
                        ...
                        if(!monte_carlo_mult_comp(zstat,threshold,extent,n_threshold_extent,string_z,ifh,mode,tstat_p,string_t,
                            swapbytes,pstr,lc_test_type[t]==0?avg1:(double*)NULL,avgstr,mcmc,ap))exit(-1);
                        }
                    if(lc_T_uncorrected || lc_T_monte_carlo) {

                        #if 0
                        for(j=0;j<lenbrain;j++) temp_float[brnidx[j]] = (float)dferror_stack[j];
                        w1->temp_double=dferror_stack;
                        #endif
                        //START190220
                        for(j=0;j<lenbrain;j++) temp_float[brnidx[j]] = (float)dferror_stack_p[j];
                        w1->temp_double=dferror_stack_p;
                    ...
                    if(!nharolds_num) {
                        ...
                        if(lc_test_type[t] == 2) {
                            ...
                            //START190220
                            fprintf(fprn,"%s\n",!i?"EQUAL VARIANCES":"UNEQUAL VARIANCES");
                        ...
                        if(region_names) {
                            for(j=0;j<lenvol;j++) {
                                ...
                                //START190220
                                fprintf(fprn,"%s%*.4f %8.4f %g\n",region_names->files[j],(int)(region_names->max_length-
                                    region_names->strlen_files[j]+9),tstat_p[j],zstat[j],tstat_p[j]>0.?
                                    gsl_cdf_tdist_Q(tstat_p[j],dferror_stack_p[j]):gsl_cdf_tdist_P(tstat_p[j],dferror_stack_p[j]));
                        ...
                        if(lc_test_type[t] >= 3) {
                            ...
                            #endif
                            //START190220
                            if(dferror_stack_p[0]<10.) k=1;
                            else if(dferror_stack_p[0]<100.) k=2;
                            else if(dferror_stack_p[0]<1000.) k=3; 
                            else k=4;
                            fprintf(fprn,"%-*sr        T(df=%d)%*s       p(two-tailed)\n",(int)(region_names->max_length+6),string,
                                (int)dferror_stack_p[0],k,"Z");
                            ...
                            for(j=0;j<lenvol;j++) {
                                if(nsubjects1[j]) {
                                    ...
                                    //START190220
                                    fprintf(fprn,"%-3d %s%*.4f %8.4f %8.4f  %g\n",harolds_num[j],region_names->files[j],
                                        (int)(region_names->max_length-region_names->strlen_files[j]+9),r[j],tstat_p[j],zstat[j],
                                        tstat_p[j]>0.?gsl_cdf_tdist_Q(tstat_p[j],dferror_stack_p[j]):
                                        gsl_cdf_tdist_P(tstat_p[j],dferror_stack_p[j]));
                            ...
                            for(j=0;j<lenvol;j++) {
                                fprintf(fprn,"%-3d %s%*.4f %8.4f %8.4f %8.4f %g\n",harolds_num[j],region_names->files[j],
                                    (int)(region_names->max_length-region_names->strlen_files[j]+9),avg1[j],sem_p[j],tstat_p[j],
                                    zstat[j],tstat_p[j]>0.?gsl_cdf_tdist_Q(tstat_p[j],dferror_stack_p[j]):
                                    gsl_cdf_tdist_P(tstat_p[j],dferror_stack_p[j]));
                                }
                    ...
                    if(lc_test_type[t]<3){
                        fprintf(fprn1,"%s",td->name[t]);
                        if(lc_test_type[t]==2)fprintf(fprn1,"_%s",!i?"eqvar":"uneqvar"); 
                        for(j=0;j<lenvol;j++)fprintf(fprn1,"\t%.6f %.6f",lc_test_type[t]<2?avg1[j]:numerator[j],sem_p[j]);
                        fprintf(fprn1,"\n");
                        }
