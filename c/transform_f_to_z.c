/* Copyright 12/31/99 Washington University.  All Rights Reserved.
   transform_f_to_z.c  $Revision: 1.10 $ */
...
if(argc < 3) {
    fprintf(stderr,"Usage: transform_f_to_z -fstat_file fstat.4dfp.img\n");
    fprintf(stderr,"        -fstat_file:  *fstat*.4dfp.img\n");
    exit(-1);
    }
for(i=1;i<argc;i++) {
    if(!strcmp(argv[i],"-fstat_file") && argc > i+1)
        file=argv[++i];
    }
if(!file) {
    printf("Error: Need -fstat_file\n");
    exit(-1);
    }
