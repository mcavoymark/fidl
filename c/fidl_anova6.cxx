/* Copyright 1/13/22 Washington University.  All Rights Reserved.
   fidl_anova6.cxx  $Revision: 1.1 $ */
...
double *fstatmap=NULL,*zstat=NULL,td=0.,p_val=0.,*fstatmap_nobox,voxel_threshold=0.01,*fstatmap_box,*zstat_box,*dferror_stack=NULL,
    ...
    *rstack=NULL,outputnum=0.; /* Y2ijkm,Y2ijkdot,dfuneqvar,dfeqvar */
...
if(argc < 3) {
    ...
    //START220113
    fprintf(stderr,"    -var:              Variance map for F contrast. Use in place of -glm_files\n");
    fprintf(stderr,"    -df:               Degrees of freedom for F contrast. Text file with single number. Use in place of -glm_files\n");
...
for(i=1;i<argc;i++) {
    ...
    if(!strcmp(argv[i],"-print_cov_matrix") && argc > i+1) {
        l= strlen(argv[i+1]);
        if(!(cov_matrix_dir=(char*)malloc(sizeof*cov_matrix_dir*(l+1)))) {
    ...
    if(!strcmp(argv[i],"-threshold_extent") && argc > i+1) {
        ...
        if(!(threshold=(double*)malloc(sizeof*threshold*n_threshold_extent))) {
            printf("Error: Unable to malloc threshold\n");
            exit(-1);
            }
        if(!(extent=(int*)malloc(sizeof*extent*n_threshold_extent))) {
    ...
    if(!strcmp(argv[i],"-epsilon") && argc > i+1) {
        ...
        if(!(epsilon1=(double*)malloc(sizeof*epsilon1*nepsilon))) {
            printf("Error: Unable to malloc epsilon1\n");
            exit(-1);
            }
        if(!(epsilon2=(double*)malloc(sizeof*epsilon2*nepsilon))) {
    ...
    if(!strcmp(argv[i],"-dimxyz") && argc > i+1) {
        ...
        if(!(dimxyz=(int*)malloc(sizeof*dimxyz*ndimxyz))) {
...
if(!nepsilon&&!lclmerr&&!lclmerTest){
    if(!(epsilon1=(double*)malloc(sizeof*epsilon1*ad->aa*nreg))) {
        printf("fidlError: Unable to malloc epsilon1\n");
        exit(-1);
        }
    if(!(epsilon2=(double*)malloc(sizeof*epsilon2*ad->aa*nreg))) {
...
if(lcGIGAdesign) {
    if(!(epsilon1GIGAdesign=(double*)malloc(sizeof*epsilon1GIGAdesign*ad->aa*nreg))) {
        printf("Error: Unable to malloc epsilon1GIGAdesign\n");
        exit(-1);
        }
    if(!(epsilon2GIGAdesign=(double*)malloc(sizeof*epsilon2GIGAdesign*ad->aa*nreg))) {
        printf("Error: Unable to malloc epsilon2GIGAdesign\n");
        exit(-1);
        }
    if(!(chatGIGAdesign=(double*)malloc(sizeof*chatGIGAdesign*ad->aa*nreg))) {
...
if(!lc_box_correction_only){
    if(!lclmerr&&!lclmerTest)if(!(cellmeansi=(int*)malloc(sizeof*cellmeansi*ad->num_sources))){
    ...
    if(lc_statview) {
        ...
        if(lcWCM||lcssftest) {
            ...
            if(!(cmm=(CellMeansModel*)malloc(sizeof*cmm))) {
                printf("fidlError: Unable to malloc cmm\n");
                exit(-1);
                }
            cmm->ntreatments=ad->num_cells_notRANDOM;
            if(!(cmm->uhat=(double*)malloc(sizeof*cmm->uhat*cmm->ntreatments))) {
                printf("fidlError: Unable to malloc cmm->uhat\n");
                exit(-1);
                }
            if(!(cmm->weight=(double*)malloc(sizeof*cmm->weight*cmm->ntreatments))) {
                printf("fidlError: Unable to malloc cmm->weight\n");
                exit(-1);
                }
            if(!(cmm->n=(int*)malloc(sizeof*cmm->n*cmm->ntreatments))) {
        ...
        if(lcGIGA||lcGIGAdesign) {
            GIGAcellmeansn=1;
            if(!(GIGAcellmeansi=(int*)malloc(sizeof*GIGAcellmeansi*GIGAcellmeansn))) {
            ...
            if(!(GIGAcmm=(CellMeansModel*)malloc(sizeof*GIGAcmm))) {
    ...
    if(regional_anova_name) {
        ...
        if(!num_region_names&&num_glm_files&&!lutf){
            ...
            if(ifh_reg->nregions) {
                if(!(region_names=(Files_Struct*)malloc(sizeof*region_names))) {
...
//START220113
if(lcssftest)if(!num_glm_files&&(!varf||!dff)){
    printf("fidlError: Need to specify either -glm_files or both -var and -df\n");
    exit(-1);
    }


if(!(glmstack=(LinearModel**)malloc(sizeof*glmstack*glmpersubmax))) {
    printf("fidlError: Unable to malloc glmstack\n");
    exit(-1);
    }
if(glm_files){
    for(j=0;j<glmpersub[0];j++) {
        if(!(glmstack[j]=read_glm(glm_files->files[j],(int)SMALL))) {
    ...
    if(!lcssftest){
        if(filetype==(int)CIFTI){
            ...
            if(!(cifti_xmldata=(char*)malloc(sizeof*cifti_xmldata*xmlsize))) {
        ...
        if(glmpersub[0]>1) {
            if(!(gms_stack=(Grand_Mean_Struct**)malloc(sizeof*gms_stack*glmpersub[0]))) {
...
if(!lc_names_only){
    if(ad->datafiles){
        ...
        if((lcGIGA||lcGIGAdesign)&&cellmeansf) {
            if(!(avg=(double*)malloc(sizeof*avg*lenvol_whole*ad->num_cells_notRANDOM))) {
                printf("fidlError: Unable to malloc avg\n");
                exit(-1);
                }
            }
        if(!(ms.get_mask(mask_file,lenvol_whole,(int*)NULL,(LinearModel*)NULL,0)))exit(-1);
        ...
        if(!num_region_names&&!lutf){
            ...
            if(!voloption) {
                ...
                if(!ap) {
                    ...
                    //START220205
                    ifh1=lcssftest?(atlas[0]?NULL:(num_glm_files?glmstack[0]->ifh:ms.ifh)):(filetype==(int)NIFTI?ifh:(output?ifh:NULL));
            ...
            if(!(ifh->datades=(char*)malloc(sizeof*ifh->datades*14))) {
            ...
            if(mask_file) {
                ...
                if(!(ifh->mask=(char*)malloc(sizeof*ifh->mask*(strlen(mask_file)+1)))) {
    ...
    if(!lc_box_correction_only && !lccleanup_only) {
        if(!(nsubjects=(int*)malloc(sizeof*nsubjects*(text||lc_statview||!ad->datafiles?1:lenbrain)))) {
            printf("Error: Unable to malloc nsubjects\n");
            exit(-1);
            }
        if(!(mm=(Memory_Map**)malloc(sizeof*mm*chunk))) {
            printf("Error: Unable to malloc mm\n");
            exit(-1);
            }
        if(!lcssftest) if(!(dferror=(double*)malloc(sizeof*dferror*lenvol_whole*ad->aa))) {
            printf("Error: Unable to malloc dferror\n");
            exit(-1);
            }
        if(!(dferror_stack=(double*)malloc(sizeof*dferror_stack*lenvol_whole))) {
            printf("Error: Unable to malloc dferror_stack\n");
            exit(-1);
            }
        if(!(dfeffect_stack=(double*)malloc(sizeof*dfeffect_stack*lenvol_whole))) {
            printf("Error: Unable to malloc dfeffect_stack\n");
            exit(-1);
            }
        if(!(temp_double=(double*)malloc(sizeof*temp_double*(lcssftest?how_many:lenvol_whole)))) {
            printf("Error: Unable to malloc temp_double\n");
            exit(-1);
            }
        if(!(temp_float=(float*)malloc(sizeof*temp_float*how_many))) {
        ...
        if(lc_statview) {
            ...
            if(!fprn) {
                if(!(zstat=(double*)malloc(sizeof*zstat*lenvol_whole))) {
                    printf("Error: Unable to malloc zstat\n");
                    exit(-1);
                    }
                if(!(work=(int*)malloc(sizeof*work*lenvol_whole))) {
                    printf("Error: Unable to malloc work\n");
                    exit(-1);
                    }
                if(lcclassical||lcWCM||lcssftest) {
                    if(!(fstatmap_box=(double*)malloc(sizeof*fstatmap_box*lenvol_whole))) {
                        printf("Error: Unable to malloc fstatmap_box\n");
                        exit(-1);
                        }
                    if(!(fstatmap_nobox=(double*)malloc(sizeof*fstatmap_nobox*lenvol_whole))) {
                        printf("Error: Unable to malloc fstatmap_nobox\n");
                        exit(-1);
                        }
                    if(!(zstat_box=(double*)malloc(sizeof*zstat_box*lenvol_whole))) {
                        printf("Error: Unable to malloc zstat_box\n");
                        exit(-1);
                        }
                    if(!(temp_float_box=(float*)malloc(sizeof*temp_float_box*how_many))) {
        ...
        if(lcclassical||lcWCM||lcssftest) {
            if(!(fstatmap=(double*)malloc(sizeof*fstatmap*ad->num_sources*lenbrain))) {
                printf("Error: Unable to malloc fstatmap\n");
                exit(-1);
                }
            for(j=0;j<ad->num_sources*lenbrain;j++) fstatmap[j] = (double)UNSAMPLED_VOXEL;
            if(lcssftest) {
                if(!(stat=(double*)malloc(sizeof*stat*how_many))) {
                ...
                ss->uneqvar=NULL;
                if(!(ss->eqvar=(double*)malloc(sizeof*ss->eqvar*lenbrain))) {
                ...
                //if(glmpersub[0]>1){
                //START220206
                if(glmpersub) if(glmpersub[0]>1){

                    if(!(ss->uneqvar=(double*)malloc(sizeof*ss->uneqvar*lenbrain))) {
                    ...
                    #endif
                    //START210510
                    if(!(ss->dfuneqvar=(double*)malloc(sizeof*ss->dfuneqvar*lenbrain))) {
                        printf("fidlError: Unable to malloc ss->dfuneqvar\n");
                        exit(-1);
                        }

                    if(!(ss->fstatuneqvar=(double*)malloc(sizeof*ss->fstatuneqvar*ad->num_sources))) {
                        printf("fidlError: Unable to malloc ss->fstatuneqvar\n");
                        exit(-1);
                        }
                    if(!(fstatmapuneqvar=(double*)malloc(sizeof*fstatmapuneqvar*ad->num_sources*lenbrain))) {
        ...
        if(lcGIGA) {
            if(!(GIGAfstatmap=(double*)malloc(sizeof*GIGAfstatmap*ad->num_sources*lenbrain))) {
                printf("Error: Unable to malloc GIGAfstatmap\n");
                exit(-1);
                }
            for(j=0;j<ad->num_sources*lenbrain;j++) GIGAfstatmap[j] = (double)UNSAMPLED_VOXEL;
            if(!(GIGAchatmap=(double*)malloc(sizeof*GIGAchatmap*ad->num_sources*lenbrain))) {
                printf("Error: Unable to malloc GIGAchatmap\n");
                exit(-1);
                }
            if(!(GIGAhtildaprimeprimemap=(double*)malloc(sizeof*GIGAhtildaprimeprimemap*ad->num_sources*lenbrain))) {
                printf("Error: Unable to malloc GIGAhtildaprimeprimemap\n");
                exit(-1);
                }
            if(!(GIGAhtildamap=(double*)malloc(sizeof*GIGAhtildamap*ad->num_sources*lenbrain))) {
                printf("Error: Unable to malloc GIGAhtildamap\n");
                exit(-1);
                }
            }
        if(lcGIGAdesign) {
            if(!(GIGAdesignfstatmap=(double*)malloc(sizeof*GIGAdesignfstatmap*ad->num_sources*lenbrain))) {
        ...
        if(lclmerr||lclmerTest){
            ...
            if(!(lmerpmap=(double*)malloc(sizeof*lmerpmap*nlmerpmap*lenbrain))){
            ...
            if(!(rstack=(double*)malloc(sizeof*rstack*chunk*(rnvox<lenbrain?rnvox:lenbrain)))){
                printf("fidlError: Unable to malloc rstack\n");
                exit(-1);
                }

            //STARRT200218
            if(!(brnidx0=(int*)malloc(sizeof*brnidx0*lenbrain))){
    ...
    if(lc_statview) {
        ...
        if(!(temp_int=(int*)malloc(sizeof*temp_int*j))) {
        ...
        if(num_glm_files) {
            ...
            if(lcssftest) {
                if(nxformfiles) {
                    if(!(t4=(float*)malloc(sizeof*t4*(size_t)T4SIZE*nxformfiles))) {
                        printf("fidlError: Unable to malloc t4\n");
                        exit(-1);
                        }
                    if(!(A_or_B_or_U=(int*)malloc(sizeof*A_or_B_or_U*nxformfiles))) {
...
//STARTHERE
                ...
                if(!(subcov=(double*)malloc(sizeof*subcov*tcs->num_tc*tcs->num_tc*ad->Perlman_levels[RANDOM]*nreg))) {
                ...
                if(lcsphericity_new||lcGIGAdesign) {
                    ...
                    if(!(Mcolsub=(int*)malloc(sizeof*Mcolsub*ad->Perlman_levels[RANDOM]))) {
                        printf("Error: Unable to malloc Mcolsub\n");
                        exit(-1);
                        }
                    if(!(Mcol=(int*)malloc(sizeof*Mcol*num_glm_files))) {
                        printf("fidlError: Unable to malloc Mcol\n");
                        exit(-1);
                        }
                    if(!(nc=(int*)malloc(sizeof*Mcol*num_glm_files))) {
                        printf("fidlError: Unable to malloc Mcol\n");
                        exit(-1);
                        }
                    ...
                    if(!(A=(double*)malloc(sizeof*A*Nrowmax*Mcolmax))) {
                        printf("fidlError: Unable to malloc A\n");
                        exit(-1);
                        }
                    i=tcs->num_tc>Mcolmax?tcs->num_tc:Mcolmax;
                    if(!(ATA=(double*)malloc(sizeof*ATA*i*i))) {
                        printf("fidlError: Unable to malloc ATA\n");
                        exit(-1);
                        }

                    if(!(ATAbig=(double*)malloc(sizeof*ATAbig*Mcolsubmax*Mcolsubmax))) {
                        printf("fidlError: Unable to malloc ATAbig\n");
                        exit(-1);
                        }
                    if(!(transform=(double*)malloc(sizeof*transform*tcs->num_tc*Mcolsubmax))) {
                        printf("fidlError: Unable to malloc transform\n");
                        exit(-1);
                        }
                    if(!(transformATA=(double*)malloc(sizeof*transformATA*tcs->num_tc*Mcolsubmax))) {
                        printf("fidlError: Unable to malloc transformATA\n");
                        exit(-1);
                        }
                    if(!(transformATAtransform=(double*)malloc(sizeof*transformATAtransform*tcs->num_tc*tcs->num_tc))) {
                        printf("fidlError: Unable to malloc transformATAtransform\n");
                        exit(-1);
                        }
                    if(!(ATAm1=(double*)malloc(sizeof*ATAm1*tcs->num_tc*tcs->num_tc))) {
                        printf("fidlError: Unable to malloc ATAm1\n");
                        exit(-1);
                        }
                    ATAm1gsl = gsl_matrix_view_array(ATAm1,tcs->num_tc,tcs->num_tc);
                    if(!(coli=(int*)malloc(sizeof*coli*glmpersubmax))) {
                        printf("fidlError: Unable to malloc coli\n");
                        exit(-1);
                        }
                    if(tc_contrast) {
                        if(!(tcstacki=(int*)malloc(sizeof*tcstacki*tcs->num_tc))) {
                            printf("fidlError: Unable to malloc tcstacki\n");
                            exit(-1);
                            }
                        }
                    perm = gsl_permutation_alloc(tcs->num_tc);
                    if(!(V=(double*)malloc(sizeof*V*tcs->num_tc*tcs->num_tc))) {
                        printf("fidlError: Unable to malloc V\n");
                        exit(-1);
                        }
                    if(!(Ssvd=(double*)malloc(sizeof*Ssvd*tcs->num_tc))) {
                        printf("fidlError: Unable to malloc Ssvd\n");
                        exit(-1);
                        }
                    if(!(worksvd=(double*)malloc(sizeof*worksvd*tcs->num_tc))) {
                    ...
                    for(Mcoli=jjj=jj=i=0;i<ad->Perlman_levels[RANDOM];Mcoli+=glmpersub[i++]) {
                        ...
                        for(j1=j=0;j<glmpersub[i];j++,jj++) {
                            ...
                            #endif
                            /*START150811*/
                            if(i)if(!(glmstack[j]=read_glm(glm_files->files[jj],1))) {
                                printf("fidlError: reading %s  Abort!\n",glm_files->files[jj]);
                                exit(-1);
                                }
                        ...
                        if(!tc_contrast) {
                            ...
                            //START220113
                            if(!(fbf=find_regions_by_file(glmpersub[i],tcs->num_tc,&nc[Mcoli],tcstacki,NULL)))exit(-1);
...
if(!lc_box_correction_only) {
    if(!lccleanup_only) {
            ...
            if(text || lc_statview || !ad->datafiles) {
                ...
                if(lccheckunsampled) {
                    if(!(subject_unsampled_count=(int*)malloc(sizeof*subject_unsampled_count*ad->Perlman_levels[0]))) {
                ...
                if(!(unsampled_voxels_idx=(int*)malloc(sizeof*unsampled_voxels_idx*num_unsampled_voxels))) {
                    printf("fidlError: Unable to malloc unsampled_voxels_idx\n");
                    exit(-1);
                    }
                if(!(lt2subjects_voxels_idx=(int*)malloc(sizeof*lt2subjects_voxels_idx*num_voxels_lt2subjects))) {
                    printf("fidlError: Unable to malloc lt2subjects_voxels_idx\n");
                    exit(-1);
                    }
                for(num_unsampled_voxels=num_voxels_lt2subjects=j=0;j<lenbrain;j++)
                    if(nsubjects[j]<ad->Perlman_levels[0]) {
                        ...
                        if(td1 > voxel_threshold) { 
                            ...
                            if(lcclassical||lcWCM||lcssftest) {
                                ...
                                if(cellmeans(ad,nsubjects[k],cmm,fprn,lccellmeans,cellmeansn,cellmeansi,&check)) {
                                    ...
                                    if(anova(ad,fprn,&epsilon1[pp],&epsilon2[pp],&dferror[k*ad->aa],kr,cmm,SSE,dfSSE,
                                        ...
                                        if(ss&&(glmpersub[0]>1))for(j=0;j<end_fstat;j++)fstatmapuneqvar[j*lenbrain+k]=ss->fstatuneqvar[j];
                                        }
                            ...
                            if(lcGIGA||lcGIGAdesign) { /*ANOVA - benefit of this clumsy coding-> can run both at once*/
                                ...
                                if(lcGIGA) {
                                    ...
                                    if(algina_guts_anova(ad,GIGAh,nsubjects[k],al->n,al->sstack,al,classical_WCM_GIGA-2,classical_WCM_GIGA,anovas)) {
                                        printf("ms.brnidx[%d]=%d\n",k,ms.brnidx[k]);
                                        if(!fprn)print_atlas_coor(ms.brnidx[k],ap);
                                        }
                                    else {
            ...
            if(fprn) {
                ...
                if(!lc_names_only){
                    if(filetype==(int)IMG){
                        ...
                        if(!(ifh->file_name=(char*)malloc(sizeof*ifh->file_name*(strlen(lme->model)+1)))) {
...

{
    ...
    for(kk=df1start,l1=m1=k=l=ll=0,i=start;i<ad->num_sources;i+=2,k++,kk++) { /*i=0 is the mean*/
        ...
        if(!cmm) {
            ...
            if(!(Cuhat=(double*)malloc(sizeof*Cuhat*h->box_dfeffect[k]))) {
                printf("fidlError: Unable to malloc Cuhat\n");
                return 1;
                }
            for(j=0;j<h->box_dfeffect[k];j++) {
                Cuhat[j] = 0.;
...
Helmert *helmert(AnovaDesign *ad,int GIGA,int design)
{
    ...
    if(!(h=(Helmert*)malloc(sizeof*h))) {
        printf("fidlError: Unable to malloc h\n");
        return NULL;
        }
    h->Cr=(double**)NULL;
    if(!(h->box_dfeffect=(int*)malloc(sizeof*h->box_dfeffect*ad->aa1))) {
        printf("fidlError: Unable to malloc h->box_dfeffect\n");
        return NULL;
        }
    if(!(box_levels=(int*)malloc(sizeof*box_levels*ad->num_factors))) {
        printf("fidlError: Unable to malloc box_levels\n");
        return NULL;
        }
    if(GIGA) {
        if(!(h->box_dfeffect_between=(int*)malloc(sizeof*h->box_dfeffect_between*ad->aa1))) {
            printf("fidlError: Unable to malloc h->box_dfeffect_between\n");
            return NULL;
            }
        if(!(box_levels_between=(int*)malloc(sizeof*box_levels_between*ad->num_factors))) {
            printf("fidlError: Unable to malloc box_levels_between\n");
            return NULL;
            }
    ...
    if(!(h->contrasts=(double*)malloc(sizeof*h->contrasts*nrow*h->ncol))) {
        printf("fidlError: Unable to malloc h->contrasts\n");
        return NULL;
        }
    ...
    #if 1
    if(GIGA) {
        if(!(h->AATstack=(double*)malloc(sizeof*h->AATstack*ad->aa1*h->ncol*h->ncol))) {
            printf("fidlError: Unable to malloc h->AATstack\n");
            return 0;
            }
...
void cmm_design(AnovaDesign *ad,CellMeansModel *cmm)
{
    ...
    if(!(cmm->nind=(int*)malloc(sizeof*cmm->nind*(ad->aa1)))) {
        printf("fidlError: Unable to malloc cmm->nind\n");
        exit(-1);
        }
    for(n=0,l=2;l<ad->num_sources;l+=2,n++) {
        cmm->nind[n] = 1;
...
double *get_Sstackinv(int j,LinearModel **glmstack,TC *tcs,int tc_contrast,AnovaDesign *ad,Helmert *h,int glmpersub0)
{
    ...
    if(!(Sstackinv=(double*)malloc(sizeof*Sstackinv*k))) {
    ...
    if(!(ATAm1xtransformT=(double*)malloc(sizeof*ATAm1xtransformT*Mcol*tcs->num_tc))) {
        printf("fidlError: Unable to malloc ATAm1xtransformT\n");
        return NULL;
        }

    if(!(transformed_ATAm1=(double*)malloc(sizeof*transformed_ATAm1*tcs->num_tc*tcs->num_tc))) {
        printf("fidlError: Unable to malloc transformed_ATAm1\n");
        return NULL;
        }
    if(!(Cxtransformed_ATAm1=(double*)malloc(sizeof*Cxtransformed_ATAm1*l*tcs->num_tc))) {
        printf("fidlError: Unable to malloc Cxtransformed_ATAm1\n");
        return NULL;
        }
    if(!(S=(double*)malloc(sizeof*S*l*l))) {
...
Classical *classical(AnovaDesign *ad)
{
    int i,ii,error;
    Classical *c;
    if(!(c=(Classical*)malloc(sizeof*c))) {
        printf("fidlError: Unable to malloc c\n");
        return NULL;
        }
    if(!(c->error=(int*)malloc(sizeof*c->error*ad->aa))) {
        printf("fidlError: Unable to malloc c->error\n");
        return NULL;
        }
    if(!(c->nerror=(int*)malloc(sizeof*c->nerror*ad->aa))) {
        printf("fidlError: Unable to malloc c->nerror\n");
        return NULL;
        }
...
Algina *algina_init(AnovaDesign *ad,Helmert *h){
    ...
    Algina *al;
    if(!(al=(Algina*)malloc(sizeof*al))) {
        printf("fidlError: Unable to malloc al\n");
        return NULL;
        }
    if(!(al->G=(double*)malloc(sizeof*al->G*ad->num_cells_notRANDOM*ad->num_cells_notRANDOM))) {
        printf("fidlError: Unable to malloc al->G\n");
        return NULL;
        }
    al->gslG = gsl_matrix_view_array(al->G,ad->num_cells_notRANDOM,ad->num_cells_notRANDOM);
    if(!(al->Sstar=(double*)malloc(sizeof*al->Sstar*ad->num_cells_notRANDOM*ad->num_cells_notRANDOM))) {
        printf("fidlError: Unable to malloc al->Sstar\n");
        return NULL;
        }
    al->gslSstar = gsl_matrix_view_array(al->Sstar,ad->num_cells_notRANDOM,ad->num_cells_notRANDOM);
    if(!(al->GSstar=(double*)malloc(sizeof*al->GSstar*ad->num_cells_notRANDOM*ad->num_cells_notRANDOM))) {
        printf("fidlError: Unable to malloc al->GSstar\n");
        return NULL;
        }
    ...
    if(!(al->ATs=(double*)malloc(sizeof*al->ATs*j*h->ncol))) {
        printf("fidlError: Unable to malloc al->ATs\n");
        return NULL;
        }
    if(!(al->ATsA=(double*)malloc(sizeof*al->ATsA*j*j))) {
        printf("fidlError: Unable to malloc al->ATsA\n");
        return NULL;
        }
    for(j=i=0;i<ad->aa1;i++) if(h->box_dfeffect_between[i]>j) j=h->box_dfeffect_between[i];
    if(!(al->CrT=(double*)malloc(sizeof*al->CrT*j*h->ncol_between))) {
        printf("fidlError: Unable to malloc al->CrT\n");
        return NULL;
        }
    if(!(al->Cr=(double*)malloc(sizeof*al->Cr*j*h->ncol_between))) {
        printf("fidlError: Unable to malloc al->Cr\n");
        return NULL;
        }
    if(!(al->CrTNm1=(double*)malloc(sizeof*al->CrTNm1*j*h->ncol_between))) {
        printf("fidlError: Unable to malloc al->CrTNm1\n");
        return NULL;
        }
    if(!(al->CrTNm1Cr=(double*)malloc(sizeof*al->CrTNm1Cr*j*j))) {
        printf("fidlError: Unable to malloc al->CrTNm1Cr\n");
        return NULL;
        }
    if(!(al->V=(double*)malloc(sizeof*al->V*j*j))) {
        printf("fidlError: Unable to malloc al->V\n");
        return NULL;
        }
    if(!(al->Sgsl=(double*)malloc(sizeof*al->Sgsl*j))) {
        printf("fidlError: Unable to malloc al->Sgsl\n");
        return NULL;
        }
    if(!(al->work=(double*)malloc(sizeof*al->work*j))) {
        printf("fidlError: Unable to malloc al->work\n");
        return NULL;
        }
    if(!(al->CrCrTNm1Crm1=(double*)malloc(sizeof*al->CrCrTNm1Crm1*j*h->ncol_between))) {
        printf("fidlError: Unable to malloc al->CrCrTNm1Crm1\n");
        return NULL;
        }
    if(!(al->CrCrTNm1Crm1CrT=(double*)malloc(sizeof*al->CrCrTNm1Crm1CrT*h->ncol_between*h->ncol_between))) {
        printf("fidlError: Unable to malloc al->CrCrTNm1Crm1CrT\n");
        return NULL;
        }
    al->gslCrCrTNm1Crm1CrT = gsl_matrix_view_array(al->CrCrTNm1Crm1CrT,h->ncol_between,h->ncol_between);
    if(!(al->scalarAAT=(double*)malloc(sizeof*al->scalarAAT*h->ncol*h->ncol))) {
        printf("fidlError: Unable to malloc al->scalarAAT\n");
        return NULL;
        }
    al->gslscalarAAT = gsl_matrix_view_array(al->scalarAAT,h->ncol,h->ncol);

    if(!(al->Sp=(double*)malloc(sizeof*al->Sp*h->ncol*h->ncol))) {
        printf("fidlError: Unable to malloc al->Sp\n");
        return NULL;
        }
    al->gslSp=gsl_matrix_view_array(al->Sp,h->ncol,h->ncol);
    if(!(al->S=(double*)malloc(sizeof*al->S*h->ncol*h->ncol))) {
        printf("fidlError: Unable to malloc al->S\n");
        return NULL;
        }
    if(!(al->s=(double*)malloc(sizeof*al->s*h->ncol*h->ncol))) {
        printf("fidlError: Unable to malloc al->s\n");
    ...
    if(!(al->vec=(double*)malloc(sizeof*al->vec*h->ncol))) {
        printf("fidlError: Unable to malloc al->vec\n");
        return NULL;
        }
    if(!(al->sstack=(double*)malloc(sizeof*al->sstack*h->ncol_between*h->ncol*h->ncol))) {
        printf("fidlError: Unable to malloc al->sstack\n");
        return NULL;
        }
    if(!(al->ag=(double*)malloc(sizeof*al->ag*h->ncol_between))) {
        printf("fidlError: Unable to malloc al->ag\n");
        return NULL;
        }
    if(!(al->bg=(double*)malloc(sizeof*al->bg*h->ncol_between))) {
        printf("fidlError: Unable to malloc al->bg\n");
        return NULL;
        }
    if(!(al->n=(int*)malloc(sizeof*al->n*h->ncol_between))) {
        printf("fidlError: Unable to malloc al->n\n");
        return NULL;
        }

    if(!(al->nold=(int*)malloc(sizeof*al->nold*h->ncol_between))) {
        printf("fidlError: Unable to malloc al->nold\n");
        return NULL;
        }
    for(j=0;j<h->ncol_between;j++) al->nold[j]=-1;

    if(!(al->nsubpergroup=(int*)malloc(sizeof*al->nsubpergroup*h->ncol_between))) {
        printf("fidlError: Unable to malloc al->nsubpergroup\n");
        exit(-1);
        }
    for(j=0;j<h->ncol_between;j++) al->nsubpergroup[j]=0;
    if(!(al->uhat=(double*)malloc(sizeof*al->uhat*ad->withprod))) {
        printf("fidlError: Unable to malloc al->uhat\n");
        exit(-1);
        }
    if(!(al->uhat_rearranged=(double*)malloc(sizeof*al->uhat_rearranged*ad->num_cells_notRANDOM))) {
        printf("fidlError: Unable to malloc al->uhat\n");
        exit(-1);
        }
    al->gsluhat_rearranged=gsl_vector_view_array(al->uhat_rearranged,ad->num_cells_notRANDOM);
    if(!(al->VecXbarCrCrTNm1Crm1CrTkronprodAAT=(double*)malloc(sizeof*al->VecXbarCrCrTNm1Crm1CrTkronprodAAT*ad->num_cells_notRANDOM))) {
        printf("fidlError: Unable to malloc al->VecXbarCrCrTNm1Crm1CrTkronprodAAT\n");
        exit(-1);
        }
    ...
    if(!(al->addressord=(int*)malloc(sizeof*al->addressord*ad->Perlman_levels[0]*ad->withprod))) {
        printf("fidlError: Unable to malloc al->addressord\n");
        exit(-1);
        }
    if(!(al->data=(double*)malloc(sizeof*al->data*ad->Perlman_levels[0]*ad->withprod))) {
        printf("fidlError: Unable to malloc al->data\n");
        exit(-1);
        }
    if(!(alwhichgroupi=(int*)malloc(sizeof*alwhichgroupi*ad->Perlman_levels[0]*ad->withprod))) {
        printf("fidlError: Unable to malloc alwhichgroupi\n");
        exit(-1);
        }
    if(!(aladdress=(int*)malloc(sizeof*aladdress*ad->Perlman_levels[0]*ad->withprod))) {
        printf("fidlError: Unable to malloc aladdress\n");
        exit(-1);
        }
    for(j=0;j<ad->Perlman_levels[0]*ad->withprod;j++) aladdress[j]=-1;
    if(!(alntreatpersub=(int*)malloc(sizeof*alntreatpersub*ad->Perlman_levels[0]))) {
        printf("fidlError: Unable to malloc alntreatpersub\n");
        exit(-1);
        }
...
Treats *get_treatment_str(AnovaDesign *ad,int nidx,int *idx){
    ...
    Treats *treats=NULL;
    if(!(treat=(int*)malloc(sizeof*treat*(nidx+1)))) {
    ...
    if(!(driver_table=(int*)malloc(sizeof*driver_table*treat[0]*nidx))) {
    ...
    if(!(treats=(Treats*)malloc(sizeof*treats))) {
        printf("fidlError: get_treatment_str Unable to malloc treats\n");
        return NULL;
        }
    treats->n=treat[0];
    if(!(treats->tptr=(char**)malloc(sizeof*treats->tptr*treat[0]))) {
        printf("fidlError: Unable to malloc treats->tptr\n");
        return NULL;
        }
    if(!(treats->t=(char*)malloc(sizeof*treats->t*len))) {
...
int statview_guts(int lenvol,Algina *al,AnovaDesign *ad,char *regional_anova_name){
    ...
    if(!(temp_double=(double*)malloc(sizeof*temp_double*ad->num_cells_notRANDOM*maxnsubpergroup))) {
        printf("Error: Unable to malloc temp_double\n");
        exit(-1);
        }
    ...
    if(ad->nbetween) {
        if(!(betprodptr=(int*)malloc(sizeof*betprodptr*ad->betprod))) {
...
SS *init_ss(Helmert *h){
    ...
    if(!(ss=(SS*)malloc(sizeof*ss))) {
        printf("Error: Unable to malloc ss\n");
        return NULL;
        }
    if(!(ss->ybar=(double*)malloc(sizeof*ss->ybar*max))) {
        printf("Error: Unable to malloc ss->ybar\n");
        return NULL;
        }
    if(!(ss->Sinvybar=(double*)malloc(sizeof*ss->Sinvybar*max))) {
        printf("Error: Unable to malloc ss->ybar\n");
        return NULL;
        }
    return ss;
...
LME *init_lme(int adaa0,char *scratchdir,char *output){
    ...
    if(!(lme=(LME*)malloc(sizeof*lme))) {
        printf("fidlError: Unable to malloc lme\n");
        return NULL;
        }
    if(!(lme->rdriver=(char*)malloc(sizeof*lme->rdriver*(len+strlen(driver)+1)))) {
    ...
    if(!(lme->rscript=(char*)malloc(sizeof*lme->rscript*(len+strlen(script)+1)))) {
...
char **get_lmef(AnovaDesign *ad,int *lmerl,int nlmef){
    ...
    if(!(lmef=(char**)malloc(sizeof*lmef*nlmef))){
        printf("Error: Unable to malloc lmef\n");
        return NULL;
        }
    if(!(lmef[0]=(char*)calloc(cnt,sizeof*lmef[0]))){
...
int get_rscript(AnovaDesign *ad,char *scratchdir,LME *lme,int chunk,int regionalanalysis,int lc_names_only){
    ...
    if(regionalanalysis){
        if(!(hdrl=(size_t*)malloc(sizeof*hdrl*nhdr))) {
...
char** get_rscript_lmerTest(AnovaDesign *ad,char *scratchdir,LME *lme,int chunk,int regionalanalysis,int lc_names_only,char *rpath,
    ...
    if(!(lmef=(char**)malloc(sizeof*lmef*lme->nlmef))){
    ...
    if(regionalanalysis){
        if(!(lme->lmerp=(int*)malloc(sizeof*lme->lmerp))){
        ...
        if(!(lme->namesfixef=(char**)malloc(sizeof*lme->namesfixef*lme->lmerp[0]))){
            printf("fidlError: get_rscript_lmerTest Unable to malloc lme->namesfixef\n");
            return NULL;
            }
        if(!(lme->namesfixef[0]=(char*)calloc(cnt,sizeof*lme->namesfixef[0]))){
...
char* get_rpath(){
        ...
        if((nstrings=count_strings_new3(line,write_back,' ',' ',&last_char,&first_char,0,' '))>=3){
            ...
            if(!strcmp("rpath",p)){
                ...
                if(!strcmp(hostname,p)){
                    ...
                    l=strlen(p);
                    if(*(p+l-1)!='/')l++;
                    if(!(rpath=(char*)malloc(sizeof*rpath*l))){
    ...
    if(!rpath){
        ...
        if(!(rpath=(char*)malloc(sizeof*rpath*(strlen(rpathd)+1)))){
            printf("fidlError: get_rpath Unable to malloc rpath\n");
            exit(-1);
            }
        strcpy(rpath,rpathd);
