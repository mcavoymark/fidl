/* Copyright 11/27/13 Washington University.  All Rights Reserved.
   fidl_language_sentoldnew_ev.c  $Revision: 1.1 $ */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <math.h>
...
{
char *txtfile=NULL,*concfile=NULL,*out=NULL,filename[MAXNAME],*strptr,*senttest=NULL,*senttest6=NULL;
...
if(argc<7) {
    ...
    /*START131127*/
    fprintf(stderr,"    -senttest:   Old version of senttest, 4 columns.\n");
    fprintf(stderr,"    -senttest6:  New version of senttest, 6 columns.\n");
    ...
    fprintf(stderr,"  NEW FORMAT: Next twelve subjects\n");
    fprintf(stderr,"    -txt:      Name of eprime derived text file.\n");
...
for(i=1;i<argc;i++) {
    ...
    /*START131127*/
    if(!strcmp(argv[i],"-senttest") && argc > i+1)
        senttest = argv[++i];
    if(!strcmp(argv[i],"-senttest6") && argc > i+1)
        senttest6 = argv[++i];

    }
if((SunOS_Linux=checkOS())==-1) exit(-1);
if(!txtfile) {
    printf("fidlError: need to specify -txt\n");
    exit(-1);
    }
if(!concfile) {
...
/*START131127*/
#if 0
if(!senttest&&!senttest6) {
    printf("fidlError: need to specify either -senttest or -senttest6\n");
    exit(-1);
    }
#endif
if(senttest&&senttest6) {
    printf("fidlError: need to specify either -senttest or -senttest6, not both\n");
    exit(-1);
    }
...
/*START131127*/
if(senttest||senttest) if(!(data=read_data(senttest?senttest:senttest6,0,0,0,0)))exit(-1);
...
if(duration>0.) {
    ...
    if(!senttest&&!senttest6) {
        ...
        printf("fidlError: This needs to be written.\n");fflush(stdout);exit(-1);
        }
    else {
        fprintf(fp,"%g plausrem plausenot plausmiss implaus implausrot\n",TR);
        for(time=starttime,i2=firstrun-1,i1=i=0;i<nfiles;i++,time+=dp->tdim[i2++]*TR,i1+=2) {
            offset = lcseparate ? i*3 : 0;
            for(time1=time+(double)skip*TR,j=0;j<data->nsubjects;time1+=duration+data->x[j++][i1+1]/1000.)
                fprintf(fp,"%g\t%g\t%g\n",time1,offset+firstevent+data->x[j][i1]-1,duration);
